...
add_section "Prohibited: Firebase Web SDK (E2.2)" 'firebase-auth|firebase-database|@firebase/'
add_section "Prohibited: CoreData in Views (E2.2)" 'NSManagedObject|NSFetchRequest|@FetchRequest'
...
# Sacred Zone Inventory
echo "## Sacred Zone Integrity (Inventory)" >> "$SCAN"
SACRED_FILES=(
  "UnifiedBalanceEngine.swift"
  "BudgetEngine.swift"
  "AppBootstrapRepository.swift"
  "AppStore.swift"
  "RecurrenceExpander.swift"
)
for f in "${SACRED_FILES[@]}"; do
  find "$ROOT" -name "$f" -print >> "$SCAN" 2>/dev/null || true
done
