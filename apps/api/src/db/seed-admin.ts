...
import { assertGlobalEmailAvailable, withIdentityEmailTransaction } from '../services/identity/email-lock.js';
import { withRetriedTenantSlug } from '../services/identity/tenant-slug.js';
...
export async function autoSeedAdmin(): Promise<void> {
  ...
  try {
    ...
    const slugBase = org.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
    const password_hash = await bcrypt.hash(password, 12);
    const tenant_id = uuidv4();
    const user_id = uuidv4();

    await withRetriedTenantSlug(slugBase, async (slug) =>
      withIdentityEmailTransaction(email, async (client, normalizedEmail) => {
        await assertGlobalEmailAvailable(client, normalizedEmail);
        await client.query('INSERT INTO tenants (id, name, slug) VALUES ($1, $2, $3)', [tenant_id, org, slug]);
        await client.query(
