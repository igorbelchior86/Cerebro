...
async function testRetryLogic() {
    ...
    await execute('INSERT INTO triage_sessions (ticket_id, status) VALUES ($1, $2)', [ticketId, 'processing']);
    ...
    try {
        ...
        if (error.name === 'LLMQuotaExceededError') {
            await (orchestrator as any).updateSessionStatus(ticketId, 'pending');
            console.log('Successfully marked session as pending.');
        }
    }

    // 3. Verify status in DB
    const session = await queryOne<{ status: string }>('SELECT status FROM triage_sessions WHERE ticket_id = $1', [ticketId]);
    console.log('Current Session Status in DB:', session?.status);

    if (session?.status === 'pending') {
        console.log('✅ PASS: Session marked as pending for retry.');
    } else {
        console.log('❌ FAIL: Session status is', session?.status);
    }
