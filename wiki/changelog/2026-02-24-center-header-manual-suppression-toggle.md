...
- Added backend manual suppression persistence in `tickets_processed` (`manual_suppressed`, `manual_suppressed_at`) and a toggle endpoint (`PATCH /email-ingestion/tickets/:ticketId/manual-suppression`).
- Updated `/email-ingestion/list` to merge automatic classifier suppression with persisted manual suppression and return `manual_suppressed`.
