...
- Updated `/ticket-intake/list` to merge automatic suppression classification with persisted manual suppression and expose `manual_suppressed`.
...
- Data: New DB columns on `tickets_processed` (`manual_suppressed`, `manual_suppressed_at`) and new endpoint `PATCH /ticket-intake/tickets/:ticketId/manual-suppression`.
