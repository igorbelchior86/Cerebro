...
- Added conservative heuristic suppression metadata (`suppressed`, reason, confidence) to `/ticket-intake/list` for obvious intake noise (`bounce`, `quarantine digest`, `marketing`).
...
- Data: `/ticket-intake/list` returns transient suppression metadata (heuristic v1) for obvious noise patterns only.
