...
- Updated API mapping in ticket intake list endpoint to return explicit `company` and `requester` fields.
...
- Data: API `/ticket-intake/list` now includes explicit `company` and `requester` output fields for sidebar consumption.
